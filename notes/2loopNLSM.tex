...
\paragraph{ZM-theory} The Lagrangian for Zakharov-Mikhailov (ZM) theory is a simple $SU(N)$ scalar theory defined in two spacetime dimensions:
\begin{eBox}
\be
...
\paragraph{YZ-Theory} Thus far, the two color-dual Lagrangians we have discussed make explicit reference to the spacetime dimension ($D=4$ for SDYM, and $D=2$ for ZM theory). This makes them poorly suited for constructing dimensionally regulated integrals in loop-level amplitudes. Here we briefly review YZ-theory, which is a honest $D$-dimensional theory first proposed by Cheung and Hsien \cite{Cheung2016prv} for the purpose of describing nonlinear sigma model (NLSM) scattering. One can understand YZ-theory as a particular dimensional reduction of Yang-Mills in $D=2d+1$ dimensions, down to $d$ dimensions. 
...
\paragraph{J-theory} There is an alternative Lagrangian formulation of NLSM constructed by Chueng and one of the authors, that instead starts from the chiral current description of NLSM:
\be
\mathcal{L} = \text{tr}[\partial U \partial U^{-1}] \equiv \text{tr}[J^\mu J_\mu]
\ee 
where $J_\mu = U\partial_\mu U^{-1}$. To build the J-theory Lagnraigna, we can think of the chiral current $J^\mu$ as a \textit{pure gauge} vector field. Thus, the field strength for $J_\mu$ must vanish, this yielding an equivalent on-shell formulation of the EOM:
\be
\Box J^\mu +g f^{abc} J^\nu J^{\mu\nu} = 0
\ee
where we have applied the gauge condition $\partial_\mu J^\mu = 0$. Adding back in a $\bar{J}^\mu$ auxiliary field as a Lagrange multiplier, we obtain the following Lagrangian that is manifestly color-dual:
\begin{eBox}
\be
\mathcal{L}^{\text{J}} = \frac{1}{2}(\partial J)(\partial \bar{J}) +g f^{abc} \bar{J}_{\mu\nu}J^\mu J^\nu
\ee
\end{eBox}
where as above, $\bar{J}_{\mu\nu} = \partial_\mu J_\nu - \partial_\nu J_\mu$. Subject to the Lorenz gauge condition, $\partial_\mu J^\mu =0$. This theory is one-to-one with the observables amplitudes of YZ-theory when only considering $Z-\bar{Z}$ on-shell states. As such, the single vertex in this theory is the same as for YZ theory: 
\be
...
Some background here on how we approach color-dual bootstrap. Building an ansatz from irreducible lorentz products (ISPs). Comments about:
\begin{itemize}
...
\be
\AcubicB{hgrey0}{}{nhpRed}{}{nhpRed}{graviton}{nhpRed}{}{nhpRed} = s_{12 }(\epsilon_{(13)} \epsilon_{(24)} - \epsilon_{(14)} \epsilon_{(23)})
\ee
