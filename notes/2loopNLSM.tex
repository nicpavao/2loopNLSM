...
\jm{So $l$ runs between legs $n$ and $1$, right?  Can we show that on all of the diagrams here?  Is the projector the normal one $P \sim \eta + \{k,q\}/k.q$?  Can we say that?  I would normally call that the on-shell state projector not the off-shell one but I could easily be missing something.}
In general, the $n$-gon vertex would be
\begin{equation}
...
\jm{I only checked 4, 6, 8 so unless someone checked 10pt I would say `through 8pt'.}
