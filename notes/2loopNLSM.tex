...
$\bar{\varepsilon} \rightarrow k$. Furthermore, a simple calculation shows that the off-shell four-point correlation function computed from this vertex satisfies the Jacobi identity of \cref{jacID}. This ensures that color-kinematics duality holds to all multiplicity at tree-level. Moreover, since the number of propagating degrees of freedom in $D$, rather than $D-2$ for gauge-invariant non-abelian vectors, taking a forward limit on the cubic graphs will yield valid color-dual representations at one-loop as well. 
