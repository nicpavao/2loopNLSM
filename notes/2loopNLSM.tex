...
numerators at both tree-level and one-loop. When plugging in appropriate on-shell states, 
we find that these numerators underpin color-dual representations of self-dual Yang-Mills, NLSM and Chern-Simons theory
through one-loop. \jm{Mention CS here?} \nhp{Added sentence} The construction of this theory, which we dub
...
potency of this theory through one-loop, the construction runs into an
...
\cite{Monteiro2011pc}, and Chern-Simons theory \cite{Ben-Shahar:2021zww}. \jm{I mentioned CS.} \nhp{Looks good}. Specifically, semi-abelian YM is simply a
...
reintroduce $D$-dimensional vertices from the full Yang-Mills Lagrangian of \cref{eq:ym-lag} that we dropped in our construction semi-abelian YM. In terms of cubic graphs, the additional interaction must necessarily have the opposite number of $\bar{A}_\mu$ and unbarred $A_\mu$ fields to that of \cref{semiYMvert}. \jm{Should we say quartic vertices with the wrong weight or something like that?  We don't have $G_L$ and $G_R$ anymore.} \nhp{Good catch -- I changed the phrasing} Reintroducing these oppositely oriented vertices allows for new internal
...
and NLSM in \cref{NLSMnGon}, along with Chern-Simons theory, which we discuss in \cref{sec:CSTheory}.  \jm{Mention CS?  Maybe something like `SDYM in \cref{SDYMnGon} 
and NLSM in \cref{NLSMnGon}, where Chern-Simons theory is discussed in \cref{sec:CSTheory}.'?} \nhp{Done.} Due to the
...
the text, self-dual Yang-Mills (SDYM) \cite{Monteiro2011pc}, two formulations of NLSM \cite{Cheung:2016prv,Cheung:2020djz,Cheung:2021zvb}, ZM \cite{Zakharov:1973pp, Cheung:2022mix}, and Chern-Simons theory \cite{Ben-Shahar:2021zww}.  \jm{I added CS} \nhp{awesome.} All
...
\ee
At first glance, this seems far removed from the semi-abelian Yang-Mills analogs in \cref{semiYMvert} and \cref{semiYMprop}. After all, semi-YM makes no reference to an antisymmetric 3-tensor and the mass dimensions of the vertices do not agree. However, to construct the full on-shell amplitudes of CS theory, we must plug in the Fourier transforms of the on-shell currents, $\bar{\varepsilon}(p)$, as described in Ref.~\cite{Ben-Shahar:2021zww}. \jm{I'm worried that the bar over epsilons is getting overused.  Howe about we use $e_\mu$ instead of $\bar{\varepsilon}_\mu$?  We could use a tilde or $\epsilon$ but those both have issues.} \nhp{I think it conventiently matches the notation of Maor and Henrik, and also lines up perfectly with semi-YM vertices. I suggest we keep.} These are related to the polarizations, $\varepsilon_i$, of the Feynman rules above as follows:
\be
