...
\abstract{
\ace{I will start working here over the weekendish}
...
\be
\qquad\qquad \stackrel{\cubic{hgrey0}{fermion2}{}{fermion2}{}{fermion2}{}}{\text{Bifundamental}}\qquad\qquad \stackrel{\Acubic{hgrey0}{}{nhpBlue}{}{nhpBlue}{}{nhpBlue}}{\text{Adjoint}}
\ee
In this review, we will present examples of this distinct kinematic representations that have appeared in the literature. Though from the get go, we stress that in this work we are interested in identifying the analog of adjoint representations of the kinematic numerators; those for which all legs are equally preferrenced by the functional dressing. The only non-trivial $S$-matrix in the literature that exhibits this property to all-loop order, is that of Zakharov-Mikhailov theory \cite{Zakharov:1973pp, Cheung:2022mix}. We will briefly review the perturbative construction of this theory below. 
...
Color-kinematics duality is a statement about amplitude integrands.
...
Many of the 1-loop results presented in \sect{} are informed by the tree-level double-copy so we will review the tree case here.
...
\ee
with legs $1$ and $n$ fixed so that the sum only contains $(n-2)!$ terms, indicating that there are additional relations (the Kleiss-Kuijf relations) amongst the partial amplitudes \cite{Kleiss:1988ne}.\footnote{The fundamental BCJ relations further reduce the basis of partial amplitudes down to $(n-3)!$ \cite{BCJ}.}
Since the Jacobi identity can be used to express any tree-level color factor in terms of the half ladders, it is enough to specify the numerators of the half ladder diagrams in order to satisfy color-kinematics duality at tree level.
...
\jm{Should we do some example like 4pt NLSM or 4pt YM and say that it's simpler than GR?  I don't think it would add much.}
...
Aside from a few special theories \citepls{}, color-dual integrands are constructed from an ansatz.
The four conditions imposed on the ansatz are summarized here and elaborated on below.
\begin{enumerate}
\item \textbf{Locality}: Diagram ``numerators'' are polynomials in momenta and
...
\item  \textbf{Graph symmetries}: The diagram numerators are invariant under the automorphisms of the diagrams \emph{including signs that compensate for color-factor sign changes}.
The diagram numerators are only a function of the diagram's topology and labeling.
\item  \textbf{Unitarity cuts}: The cuts of the ansatz must reproduce the physical unitarity cuts of the theory.
\end{enumerate}

\paragraph{Locality}

The kinematic numerators need to be $D$-dimensional, Lorentz invariant, local (polynomial) functions of the graph kinematics with the correct power counting and external states.
...
While locality is well motivated from a physical and computational standpoint, the literature has featured numerators that violate locality, break manifest Lorentz invariance, and modify naive power counting through the presence of higher spin modes \cite{Square, WeinzierlBCJLagrangian, Mogull:2015adi, FivePointN4BCJ, Johansson:2017bfl, Ben-Shahar:2022ixa, Cheung:2016prv, Cheung:2021zvb}. \jm{Alex wrote some of this citation.  I'm not sure why it broke.}
These developments suggest that relaxing locality is a viable option when the condition proves too stringent for an ansatz.
We will comment more on locality in \sect{}.

\paragraph{Color-kinematics duality}

Somewhere emphasize that Zvi et al didn't do the most general ansatz, imposed extra loop power counting constraints.
...
only for a limited ansatz that manifests locality \cite{Bern:2015ooa}.  All of the N=4 SYM constructions below 5-loop are local.
...
Want explicit Bose sym, broken in many cases.
...
Here we'll describe the why constructing the full Yang-Mills kinematic algebra (outside self-dual sector) is generically hard, and why the same reason makes NLSM beyond one-loop hard as well. It has to do with how the vector state sum mixes with the four-point contact. 

This is most easily seen by decomposing the Yang-Mill three-point interaction in terms of the following vertex in Lorenz gauge:
\be \label{YMSvert}
\Acubic{hgrey0}{}{nhpRed}{}{nhpRed}{fermion2}{} = (\varepsilon_1\varepsilon_2)(\varepsilon_3 k_1)
\ee
...
However, a more fruitful direction is to match onto the one known theory with pion power counting that manifests color-kinematics duality to all loop order, namely, Zakharov-Mikhailov theory \cite{Zakharov:1973pp, Cheung:2022mix}.
...
\subsection{All-loop No-Go Theorem}
