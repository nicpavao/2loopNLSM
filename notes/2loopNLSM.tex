...
\begin{equation}
A(...)_{\text{1-loop}} = \Delta^{(\varnothing)}_{\text{1-loop}}\, .
\end{equation}
\jm{For the tree amplitude how about we use something like}
\begin{equation}
A(1,2...,i,...n)_{\text{tree}} = \sum_{i\neq j}\Delta^{(ij)}_{\text{tree}} \sim \sum (\varepsilon_i \varepsilon) (\varepsilon k)^{n-2} (kk)^{3-n}
\end{equation}
where $i$ is the $\bar{A}_\mu$ particle.
\jm{And something similar for 1-loop?  This is one of the more important results of the paper so I'd like to add some schematic version of the $\Delta$s to make this paragraph as self-contained and clear as possible even if it introduces some redundancy.}
...
\jm{Would it be alright to omit the coupling constants in this section?  I think we've done so elsewhere.}
...
In summary, we have found that there is a tension between color-kinematics duality and an off-shell local construction of the two-loop four-point of the pure Yang-Mills integrand in $D$-dimensions.
At tree level, we demonstrated that the duality can be
...
symmetry and in this paper we have argued that off-shell locality must
be abandoned. \footnote{An example of this can already be found in the literature, where Locality is relaxed in \cite{Mogull:2015adi} in
  order to obtain color-dual two-loop integrands with on-shell 4D states.} 
...
\be
\mathcal{A}^{\text{YM}} \stackrel{\alpha'}{\longrightarrow} \mathcal{A}^{\text{OS}}t_8 F^4 \int_0^1 dx \,(1-x)^{\alpha' s_{12}-1} x^{\alpha' s_{12}-1}
\ee
The resulting Veneziano factor of Gamma functions produces rational functions of kinematics, while preserving the color-dual structure at in the color-ordering of the Park-Taylor factor in the denominator. While introducing a string world sheet is a jump from what we have found in this work, our funding certainly suggest that we must do \textit{something} to relax the constraint of
\textit{off-shell locality}, and realize locality only in the limit of
\textit{on-shell} kinematics. This could be acheived by either modifying the kinetic term or potentially something more exotic. Below we provide an overview of what one
...
\ee
One advantage of this construction is that reduces some of the $D$-dimensional complexity vector theories that arises due to the mixing between external polarization and
...
\end{equation}
where $\mathcal{O}_i$ are the gauge invariant tensor basis elements of
\cite{Bern:2017tuc,Carrasco:2019yyn}, and $ \mathcal{R}^{(i)}_{m,L}$ are
rational functions of irreducible scalar products of \cref{eq:scalar-basis}. It would be interesting if a similar construction as our toy tree-level example could be uplifted to two-loop using the globally color-dual NLSM integrand that we have computed in this work. We see this as a natural future direction worth investigating that is made possible given the results of this work.
