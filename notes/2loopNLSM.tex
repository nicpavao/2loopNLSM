...
For color-dressed cuts the grey blobs are blown up into all possible channels and all color factors $f^{abc}$ are explicitly retained on both sides of the unitarity equation \eqref{eq:SchematicCutEquation}.
For color-ordered cuts the grey blobs are only blown up in color ordered channels, all color factors in \eqref{eq:SchematicCutEquation} are suppressed, and the product of on-shell amplitudes only extends over color-ordered trees.
...
leads to an initial ansatz with 10,000 terms in the double-box, and 9,849
...
Imposing both diagram automorphisms and maximal-cut gauge invariance
on both of the basis diagrams reduces the ansatz to 2,235 terms in the
double-box and 4,133 terms in the penta-triangle.  The power-counting
restriction of BDN would reduce the counts to 2232 and 4072,
respectively.  In the original BDN work, they imposed three spanning
generalized unitarity cuts, shown in [].  Instead of directly imposing
the spanning cuts, we proceed with a relaxed version of the method of
maximal cuts [] in order to more-directly identify the location of
tension between kinematic Jacobi relations and unitarity constraints.
We find that even with the completely generic ansatz,
the representation fails to satisfy the next-to-next-to-maximal cut
shown in [].  Notably, imposing the BDN power-counting constraints
additionally causes a discrepancy with the N$^2$-max cuts show in [].
