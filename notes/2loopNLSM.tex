...
\item  \textbf{Power-counting:} The maximum power of the loop momenta in the
  numerator is bounded by Feynman-gauge Feynman rules.
\item  \textbf{Cubic diagrams}: The only diagrams with nonzero numerators are
  those with strictly cubic vertices.  This is generally a requirement
  to properly define the kinematic Jacobi relations.
\item  \textbf{Symmetries}: The diagram numerators are only a function of the
  diagram's topology and labeling. The diagram numerators are invariant under the
  automorphisms of the diagrams \emph{including signs that compensate
    for color-factor sign changes}.
\item  \textbf{Unitarity cuts}: The cuts of the ansatz must reproduce the
  physical unitarity cuts of the theory.
\item  \textbf{Color-kinematics duality}: The numerators obey some form of the
