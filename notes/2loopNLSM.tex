...
\abstract{ We make progress in understanding how color-dual
  ...
  NLSM numerators are in tension with generalizations our one-loop construction. Using our loop-level bootstrap, we also find that the most general \emph{local} ansatz
...
power counting and external states. Locality is also physically motivated since Feynman
rules by construction produce polynomial numerators. Indeed, the most natural way to guarantee locality and avoid spurious poles is to require the off-shell numerators to be polynomial functions of
...
ansatz. As such, requiring instead that locality is only realized \textit{on-shell} may suffice.
...
\!\!\! \!\!\! \!  \NMCutC =
  ...
  \end{cases}
\end{equation}
which serve as a complete basis for the full cut. Even though unitarity methods aim to generate loop-level data from
...
\begin{equation}
\simplebox = g_{\text{YM}}^4 \langle \mathcal{X}(k_1,\ell_1)\mathcal{X}(k_2,\ell_2)\mathcal{X}(k_3,\ell_3)\mathcal{X}(k_4,\ell_4)\rangle
\end{equation}
...
\begin{equation}
N^{\text{SDYM}}_{n\text{-gon}}= \nGonVector = g_{\text{YM}}^n \langle \mathcal{X}(k_1,\ell_1)\mathcal{X}(k_2,\ell_2)\cdots \mathcal{X}(k_n,\ell_n) \rangle
\end{equation}
...
scalar:
\begin{equation}
\partial_D N^{\text{SDYM}}_{n\text{-gon}}=\nGonScalar  = (2 g_{\text{YM}})^n (\ell_1 \varepsilon_1)(\ell_2 \varepsilon_2)\cdots (\ell_n \varepsilon_n)
\end{equation}
...
It is tempting to try to match the pion four-point two-loop integrand
to ZM on the cuts, but given that the cuts only probe the on-shell
four-point amplitude, which is either subtle or vanishes for ZM, it is
better to compare the off-shell numerators directly.  As local
functions, the numerators never suffer from any of the subtleties of
on-shell 2D kinematics.  Since the physical cuts are difficult to
compare, ZM theory is being used here simply as a tool to eliminate
generalized gauge freedom; ZM theory has the right power counting and
kinematic structure in 2D to compare against.  Since ZM theory is only
being used for the kinematical structure of its numerators, it is
natural to introduce a proportionality constant $z$ between the NLSM
and ZM numerators $n_\text{NLSM} \vert_{2D} = z ~ n_\text{ZM}$.  The
...
theories.\footnote{Floating the proportionality parameter $z$ is also
  ...
  dimensions \cite{Nappi:1979ig}.  When the four-point
  ...
  difference in coupling constants.}  This introduces an extra degree of
...
off-shell.  After performing the 2D matching to the following ZM basis numerators:
\be
...
\end{split}\right]
\ee
there are only 365 parameters of generalized gauge freedom. An overview of ZM theory and its color-dual construction can be found at the end of \cref{sec:CKLagrangians}. The loop momentum structure of ZM theory provides one final hint for
...
ancillary files makes exactly this choice.
...
be abandoned.\footnote{Locality is relaxed in \cite{Mogull:2015adi} in
  order to obtain color-dual two-loop integrands.} 
...
kinematics must be non-local. Thus, in order to identify valid color-dual representation of
...
As an exemplar of an off-shell non-local construction, consider the simple four-point example of a color-dual
...
$c_s+c_t+c_u=0$. In light of our findings in \cref{chefsKiss},  While we can absolutely apply a generalized gauge
transformation \cite{BCJ} to restore field theoretic locality to the
cubic numerators, it is maybe aesthetic
choice. 
...
\begin{equation}
\ZMBasisDoubleBox =\left[\begin{split}
  \langle (\ell_1+k_4) \ell_1 \rangle\langle \ell_1\ell_2 \rangle\langle \ell_2 (\ell_2+k_1) \rangle  \langle (\ell_2+k_1)(\ell_2+k_1+k_2) \rangle 
\\
\langle (\ell_1+k_3+k_4)(\ell_2+k_1+k_2) \rangle\langle (\ell_1+k_3+k_4)(\ell_1+k_4) \rangle
\end{split}\right]
\end{equation}
\begin{equation}
  \ZMBasisPentaTriangle =\left[
    \begin{split}
      \langle (\ell_1+k_4) \ell_1 \rangle\langle \ell_1\ell_2 \rangle\langle \ell_2 (\ell_2+k_1) \rangle  \langle (\ell_2+k_1)(\ell_1-k_1) \rangle 
      \\
      \langle (\ell_1+k_3+k_4)(\ell_1-k_1) \rangle\langle (\ell_1+k_3+k_4)(\ell_1+k_4) \rangle
    \end{split}
  \right]
\end{equation}
Furthermore, since ZM theory manifests color-kinematics duality
without any reference to the on-shell condition, a mass term can be
added to ZM theory without spoiling color-kinematics duality
\cite{Cheung:2022mix}. In general, if the on-shell conditions are not
used, color-kinematics duality is unaffected by the details of the
