...
\abstract{
  ...
  provides a prescriptive approach for constructing color-dual
  ...
  one-loop NLSM numerators. Using our constructive bootstrap \ace{I am
    really hesitant to call the bootstrap constructive.  While it
    technically constructs, ``constructive'' has some additional
    connotations}, we also find that the most general \emph{local}
  two-loop four-point numerator for pure Yang-Mills fails to satisfy
  global color-kinematics duality. We pinpoint the failure to the
  bow-tie unitarity cut, and discuss a path towards \emph{non-local}
...
multiple basis graphs are required, at least for a generic theory.
...
participate in the construction.  The theory will be color dual if the
cubic vertex is antisymmetric and satisfies the Jacobi identity,
specifically,
\begin{align}
...
A^c_\nu\, .
\end{equation}
Of course, higher multiplicity would require further redefinition of
the three-point vertex to satisfy Jacobi identites on all internal
...
\cref{sec:Discussion} \ace{Is this the sec you are thinking of?}. For
...
\end{equation}
The kinetic term mixes the two gauge fields and has the standard
...
We can select out both \ace{which both?} of these manifestly cubic
sectors by selecting only $\varepsilon^+\varepsilon^+$ in light-cone
gauge (i.e. SDYM) and one-minus at tree-level OR what $J$-theory and
$YZ$-theory select with their on-shell states. Indeed, the
...
in \cref{sec:CKLagrangians} \ace{I think this is where we do this
  now}, all the $J$ and $Z$ states must be on-shell in order to
...
since as constructed YZ theory mathces to NLSM for off-shell
Y-particles. We will construct these one-loop integrands in the
next section.
...
yield precisely the 4D SDYM vertex, up to an unphysical phase \ace{Can
  we provide an example?}. Of course, this has the advantage of
...
$\sum {\varepsilon_{(+l)}\varepsilon_{(-l)}}=D$, the integrand above
...
one would obtain from the Feynman rules of YZ-theory, absent the $XZ$
internal vector loop. For more background, we refer the reader to
\cref{sec:CKLagrangians}.
\begin{equation}
...
$G_L\times G_R$. Doing so will necessarily introduce internal
contractions of $\bar{A}_\mu A^\mu$,
\begin{equation}
\mathcal{A}^{\text{YM}}_{\text{2-loop}}(+,+,+,+)=\doubleBoxObstructionA +\doubleBoxObstructionB+\cdots 
\end{equation}
However, this immediately runs into the difficulty of introducing the
four-point contact needed for color-kinematics to be satisfied on all
internal edges. To do this prescriptively would require knowledge of
...
\cref{sec:bootstrap}. We will employ an ansatz constructed from
...
beyond 1-loop?''  We will tackle this question for 4-point 2-loop
scattering in NLSM before moving on to the same process in YM.

Ignoring tadpoles and BEL graphs, there are 14 cubic 4-point 2-loop
...
4-point vertices.  Notably this means that even at two loops,
unitarity is still only probing the on-shell 4-point NLSM amplitude.
...
counting, and, for 4-point and up to at least six loops, strip off a
...
One hope would be to factor out some piece of the 1-loop numerator
...
defined unambiguously from the Feynman rules.  At 4-point, only the
...
It is tempting to try to match the pion 4-point 2-loop integrand to ZM
on the cuts, but given that the cuts only probe the on-shell 4-point
...
ancillary files makes exactly this choice.\footnote{When the 4-point
...
\citepls{}.  Such double-copy constructions are important nontrivial
...
version of ``diffeomorphism invariance'': XX for sGal, and gauge
invariance of the BI photon and DBIVA supermultiplet \citepls{}.  All
...
$n_{2\text{box}} = n_{\text{cross-box}} = s^2t\, \atree$ \citepls{}.
...
impossible to construct a 2-loop integrand for semi-abelian YM theory,
that is, the theory is 1-loop exact.  Obtaining a globally color-dual
integrand beyond 1-loop then requires sacrificing more than just Bose
...
be abandoned.\footnote{Locality is relaxed in \cite{Mogull:2015adi} in
  order to obtain color-dual 2-loop integrands.}  \ace{I want to
...
numerators at 2-loop or higher.
...
\ace{I think the below is fully covered in Sections 2 and 3 now.  Can
  we cut it?}  As we will show in \sect{}, the color-dual nature of
SDYM can be understood as a direct consequence of isolating the
\textit{cubic-sector} of the full Yang-Mills amplitudes by projecting
along particular on-shell states. For this reason, the color-dual
nature depends critically on the loop order of the amplitude and does
not hold generically beyond one-loop.
...
Since ZM theory manifested color-kinematics duality without any
...
renormalizing chiral fermions \citepls{ 'tHooft}, there is ambiguity
