...
theory, notably $\mathcal{N}=4$ super-Yang Mills (sYM) at five loops \cite{FiveLoopN4, GeneralizedDoubleCopyFiveLoops, Bern:2018jmv}, color-kinematics remains a conjecture at loop
level. %Referee edit
Indeed, there are many examples in the literature where
...
\cite{Feng:2010my,Cachazo:2014xea}, as case studies to advance our
understanding of the kinematic algebra at the multi-loop level.
...
rules produce polynomial numerators.
Indeed, the most natural way to guarantee locality and avoid spurious poles is to require the off-shell numerators to be polynomial functions of kinematics.
However, insisting on a local numerator is
...
terms to include.  %Referee edit
Historically, locality has also proved a highly successful tool for simplifying kinematic numerators. %Alex, can you add more relevant references to the next two sentences?
For example, the four-point function for $\mathcal{N}=4$ super-Yang Mills admits local color-dual numerators through four loops \cite{FivePointN4BCJ}.\footnote{At five loops, the most general local ansatz has not been explored, but the double copy has been performed at five loops using alternative methods \cite{FivePointN4BCJ}.}
While locality is well motivated from a physical and complexity
...
Even though this work is concerned with non-supersymmetric theories, recall that \ansatze{} techniques have been very successful for sYM.  %Referee edit
In particular, local color-dual representations have been found through 4pt 4-loop \cite{}. %What paper should we cite?
While a local representation has not been found at five loops, the double copy has still be performed \cite{FivePointN4BCJ}.
...
the loop power counting assumptions of Ref.~\cite{Bern:2015ooa}.\footnote{As mentioned at the beginning of this section, \emph{supersymmetric} Yang-Mills is known to have a local color-dual representation at least through four loops \cite{BRY}.  A local but restricted ansatz fails at five loops, but the most general local ansatz has not been explored \cite{FivePointN4BCJ}.} %Referee edits.  What papers should we cite?
